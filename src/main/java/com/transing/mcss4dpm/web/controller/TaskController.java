...
//        没有task,解析子任务条数，往task_sub里添加数据
        ...
        System.out.println("XXXX: remote_mcss : travelParams " + travelParams);
        ...
        String crawlWay = jsonObject.getString("crawlWay");
        System.out.println("xxxxxx  crawlWay:" + crawlWay);
        ...
        if (taskId == null) {
            ...
            task.setTravelParams(travelParams);
            task.setCrawlWay(crawlWay);
    ...
    public preservePO preserveData(
        ...
        if (null != firstObject) {
            ...
            if (!storageTypeTable.equalsIgnoreCase("")) {
                ...
                SubTask subTask = taskService.getSubTaskBySubId(subTaskId);
                List<Task> taskList = taskService.getTaskByTaskId(subTask.getTaskId());
                System.out.println("XXXX: remote_mcss : taskList " + JSON.toJSONString(taskList));
                if (taskList != null && taskList.size() > 0) {
                    Task task = taskList.get(0);
                    String projectId = task.getProjectId();
                    String firstDetailId = task.getFirstDetailId();
                    String detailId = task.getDetailId();
                    String travelParams = task.getTravelParams();
                    String crawlWay = task.getCrawlWay();
                    ...
                    // 下面是
                    JSONObject jsonObjectTravelParams = (JSONObject) JSON.parse(travelParams);
                    String batchNo = null;
                    String preRun = null;
                    String md5String = firstDetailId;
                    if (jsonObjectTravelParams != null) {
                        batchNo = jsonObjectTravelParams.getString("batchNo");
                        preRun = jsonObjectTravelParams.getString("preRun");
                        System.out.println("XXXXXX: batchNo == " + batchNo);
                        System.out.println("XXXXXX: preRun == " + preRun);
                        Iterator<String> iterator = jsonObjectTravelParams.keySet().iterator();
                        try {
                            while (iterator.hasNext()) {
                                String key = iterator.next();
                                if (("flowDetailId").equals(key)) {
                                    md5String += firstDetailId;
                                } else {
                                    md5String += jsonObjectTravelParams.getString(key);
                                }
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                    ...
                    System.out.println("XXXXXX: md5String ==" + md5String);
                    if (mapListJson != null) {
                        int intValue = 1;
                        int listSize = mapListJson.size();
                        for (Map<String, Object> map : mapListJson) {
                            map.put("detailId", detailId);
                            map.put("projectID", Integer.parseInt(projectId));
                            // 当存在batchNo时，说明是看看直播，这个时候是不需要去重的，要让uniqueValue每个值都是不一样的
                            if (batchNo == null) {  // 为非看看直播
                                if (listSize > 1){
                                    System.out.println("为非看看直播但是list有多条数据");
                                    map.put("uniqueValue", MD5Util.encrypt(md5String + formerUrl + projectId + intValue));
                                    System.out.println("xxxxxxx: md5 组合为：" + md5String + formerUrl + projectId + intValue);
                                }else {
                                    System.out.println("非看看直播但是list只有一条数据");
                                    map.put("uniqueValue", MD5Util.encrypt(md5String + formerUrl + projectId));
                                    System.out.println("xxxxxxx: md5 组合为：" + md5String + formerUrl + projectId);
                                }
                            } else { // 为看看直播
                                System.out.println("为看看直播");
                                map.put("uniqueValue", MD5Util.encrypt(md5String + formerUrl + projectId + intValue));
                                System.out.println("xxxxxxx: md5 组合为：" + md5String + formerUrl + projectId + intValue);
                            }
                    ...
                    if (!jsonParam.isEmpty() && !("").equals(jsonParam) && !("[]").equals(jsonParam)) {
                        if (("process").equals(crawlWay) && !("盖茨拜耳").equals(task.getDataSource())) {
                            try {
                    ...
                    dpmMap.put("workFlowId", workFlowId);
                    dpmMap.put("travelParams", travelParams);
                    CallRemoteServiceUtil.callRemoteService(this.getClass().getName(), dpmServer + getAcceptCallbackUrl, "post", dpmMap);
                    ...
                    dpmTaskNimMap.put("travelParams", travelParams);
...
//                    esWeixinArticle.setCommentlist(commentStr);
    ...
    public void testThread(@RequestParam(value = "registServer", required = true) @ApiParam(value = "registServer", required = true) String registServer,
        ...
        Map<String, String> param = new HashMap<>();
        param.put("registServer", registServer);
        List<DevicesInfoBo> devicesInfoBos = new ArrayList<>();
        List<DevicesInf> devicesInfList = taskService.getDevicesList(param);
        List<String> deviceNames = new ArrayList<>();
        // 将查询出来的数据封装
        for (DevicesInf devicesInf : devicesInfList) {
            DevicesInfoBo devicesInfoBo = new DevicesInfoBo();
            devicesInfoBo.setId(devicesInf.getId());
            devicesInfoBo.setDevicesName(devicesInf.getDevicesName());
            devicesInfoBo.setSerialNumber(devicesInf.getSerialNumber());
            devicesInfoBo.setRegistServer(registServer);
            devicesInfoBo.setFuncation(devicesInf.getFuncation());
            deviceNames.add(devicesInf.getDevicesName());
            devicesInfoBos.add(devicesInfoBo);
        }
        System.out.println("=====================下面是第一次的对象===========================");
        System.out.println(JSON.toJSONString(devicesInfoBos));
        System.out.println("=====================上面是第一次的对象===========================");
        System.out.println("=====================下面是第一次设备名字的集合===========================");
        System.out.println(JSON.toJSONString(deviceNames));
        System.out.println("=====================上面是第一次设备名字的集合===========================");
        Future<List<DevicesInf>> future = Executors.newSingleThreadExecutor().submit(() -> {
            ...
            while (true) {
                ...
                List<DevicesInf> devicesInfList1 = taskService.getDevicesList(param);

                if (devicesInfList1.size() > devicesInfoBos.size()) {
                    for (DevicesInf devicesInf : devicesInfList1) {
                        // 如果设备名字不在设备集合中，将该条数据添加到集合中
                        if (!deviceNames.contains(devicesInf.getDevicesName())) {
                            DevicesInfoBo devicesInfoBo = new DevicesInfoBo();
                            devicesInfoBo.setId(devicesInf.getId());
                            devicesInfoBo.setDevicesName(devicesInf.getDevicesName());
                            devicesInfoBo.setSerialNumber(devicesInf.getSerialNumber());
                    ...
                    return devicesInfList1;
                }
            }
        });
        try {
            List<DevicesInf> devicesInfs1 = future.get();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
        }
        // 如果数据库增添了一天数据，将增添的数据放在集合里面
        future.notify();
