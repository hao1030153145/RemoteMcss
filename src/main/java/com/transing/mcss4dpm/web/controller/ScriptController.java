...
//            }
    ...
    public ClickCrawlPO saveCrawlSrc(
        ...
        if (scriptInfoBO != null) {
            ...
            if (baseSaoHDFS.existsFile(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/src.txt")) {
                ...
                baseSaoHDFS.uploadFile(srcByte, baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror/" + fileName + ".txt");
                // 获得该步骤下的所有文件列表
                if (baseSaoHDFS.existsFile(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/" + step + "/")) {
                    list = baseSaoHDFS.listFiles(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/" + step + "/");
                }

                // 同时获取对应的镜像文件列表
                List<String> list2 = baseSaoHDFS.listFiles(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror/");
                ...
                JSONArray objects = (JSONArray) JSONArray.toJSON(list3);
                String newPath = String.valueOf(objects);

                clickCrawlPO.setPath(newPath);
                System.out.println("获取到了文件列表的个数》》》》》》》》》》》》》》真实路径下文件数=" + list.size() + "镜像文件数=" + list2.size());
                clickCrawlPO.setCount(list.size() + list2.size());
            }
        } else {
            System.out.print("1");
            throw new WebException(MySystemCode.GET_SCRTPT_INFO_NULL);
    ...
    @RequestMapping(value = "/saveReallyCrawlSrc.json", method = RequestMethod.GET)
    @ResponseBody
    @ApiOperation(value = "点击保存规则路径时候把镜像文件存入真正路径", position = 0)
    public ClickCrawlPO saveReallyCrawlSrc(
            @RequestParam(value = "scriptId", required = true) @ApiParam(value = "scriptId", required = true) String scriptId,
            @RequestParam(value = "deviceId", required = true) @ApiParam(value = "deviceId", required = true) String deviceId,
            @RequestParam(value = "step", required = true) @ApiParam(value = "step", required = true) String step,
            HttpServletRequest req, HttpServletResponse res) {
        ...
        String baseUrl = WebUtil.getBaseFileByEnv();
        try {
            baseSaoHDFS.mkDirs(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/" + step + "/");
            baseSaoHDFS.mkDirs(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror/");
        } catch (Exception e) {
            System.out.println("创建文件夹失败》》》》》》》》》》》》》》》》》》》》》》》");
        }
        if (baseSaoHDFS.existsFile(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror/")) {
            System.out.println("有文件: ");
            List<String> scrList = baseSaoHDFS.listFiles(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror/");
            if (scrList.size() > 0){
                for (String s : scrList) {
                    byte[] srcByte = baseSaoHDFS.downloadFile(s);
                    String fileName = scriptId + "_" + step + "_" + (new Date().getTime());
            ...
            if (list.size() > 0) {
                ...
                System.out.println("获取到了文件的路径以及个数》》》》》》》》》》》》路径为" + newPath);
                System.out.println("获取到了文件的路径以及个数》》》》》》》》》》》》个数为" + list.size());
            } else {
                clickCrawlPO.setPath(null);
                clickCrawlPO.setCount(0);
                System.out.println("没有获取到了文件的路径以及个数》》》》》》》》》》》》");
            }
    ...
    @RequestMapping(value = "/deleteMirrorFile.json", method = RequestMethod.GET)
    @ResponseBody
    @ApiOperation(value = "deleteMirrorFile", position = 0)
    public CommonPO saveCrawlSrc(@RequestParam(value = "scriptId", required = true) @ApiParam(value = "scriptId", required = true) String scriptId,
                                 @RequestParam(value = "deviceId", required = true) @ApiParam(value = "deviceId", required = true) String deviceId,
                                 HttpServletRequest req, HttpServletResponse res) {
        ...
        if (baseSaoHDFS.existsFile(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror")) {
            System.out.print("存在镜像文件并清空镜像文件》》》》》》》》》》》》》》》》");
            baseSaoHDFS.deleteFile(baseUrl + "/screen_src" + "/" + scriptId + "/" + deviceId + "/mirror", true);
        }
        commonPO.setCode(0);
        return commonPO;
    }
